...
//  2. 测试 bundle 里的 vectors/ —— 即 QrBinaryTests/vectors，由同步组自动纳入资源；
...
//  QrBinaryTests 的 "Verify Protocol Vectors" 构建阶段负责校验两份副本一致，
//  不一致直接报构建错误。更新向量后执行：
//      cp protocol/vectors/* receiver/QrBinaryTests/vectors/
//
...
enum Vectors {
    ...
    static var repoDirectory: URL {
        URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()      // QrBinaryTests
